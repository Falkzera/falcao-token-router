...
- `build.ps1` — o instalador, igual na CI: o `router.exe` de release vira o sidecar
  ...
  `installer.nsi` que o NSIS compilou leva o `router.exe` e o exe com o `mainBinaryName` —
  sem precisar de 7-Zip. Imprime o caminho e os tamanhos. Põe o `cargo` do rustup no `PATH` só
  durante o build (o `tauri build` também o chama).
